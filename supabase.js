// ════════════════════════════════════════════════════════
// CRYPTOKASINO — supabase.js  v2.1
// DEMO/REAL SEPARATION — demo nikdy nejde do DB
//
// OPRAVY v2.1:
//  - getGameSettings: game_id → id (spravny nazev sloupce v DB)
//  - logBet: game_id → id, spravne nazvy sloupcu
//  - game_settings: total_wagered_btc, total_paid_btc, house_profit_btc
...
(async function () {
  ...
  function isDemoMode() {
    ...
    return true; // fallback = demo (bezpecne)
  }
  ...
  window.CK = {
    ...
    // Nastav game mode (vola se z kazde hry)
    setGameMode(mode) {
    ...
    // Balance — vraci POUZE realnou DB balance (0 pro neprihlasene)
    // Hry si samy spravuji demoBalance lokalne
    async getBalance() {
    ...
    async deductBet(satoshi) {
      if (isDemoMode()) { console.log('[CK] deductBet BLOCKED (demo)'); return { ok: true, demo: true }; }
    ...
    async addWin(satoshi) {
      if (isDemoMode()) { console.log('[CK] addWin BLOCKED (demo)'); return { ok: true, demo: true }; }
      if (!sb || !_session) return { ok: true, demo: true };
      try {
    ...
    // OPRAVA v2.1: pouziva 'id' (ne 'game_id') a spravne nazvy sloupcu
    async logBet(game, betSatoshi, profitSatoshi, multiplier = 0, meta = {}) {
      if (isDemoMode()) { console.log('[CK] logBet BLOCKED (demo) game:', game); return; }
      if (!sb || !_session) return;
      try {
        // bets tabulka: user_id, game, bet_satoshi, profit_satoshi, multiplier, meta
        ...
        // game_settings statistiky
        // OPRAVA: sloupec 'id' (ne 'game_id'), nazvy: total_wagered_btc, total_paid_btc, house_profit_btc
        try {
        ...
        } catch (e2) { console.warn('[CK] game_settings update skip:', e2.message); }
    ...
    // ── Kompletni sazka (deduct + win + log) ─────────
    async placeBet(game, betSatoshi, profitSatoshi, multiplier = 0, meta = {}) {
      if (isDemoMode()) { console.log('[CK] placeBet BLOCKED (demo)'); return { ok: true, demo: true }; }
    ...
    // ── Jackpot ──────────────────────────────────────
    // jackpot tabulka: id=1, amount_satoshi
    async getJackpot() {
      if (!sb) return 0.001;
      try {
    ...
    async getSessionStats(game) {
      if (!sb || !_session) return null;
      try {
        const { data } = await sb.from('bets').select('bet_satoshi, profit_satoshi, multiplier')
          .eq('user_id', _session.user.id).eq('game', game)
        ...
        return { bets: data.length, wagered: totalBet/1e8, profit: totalProfit/1e8,
                 winRate: Math.round(wins/data.length*100), bestMulti: maxMulti };
      } catch { return null; }
    },
    ...
    // ── Game Settings ─────────────────────────────────
    // OPRAVA v2.1: eq('id', gameId) — ne eq('game_id', gameId)
    // Vraci data primo z DB vcetne rtp_pct, house_edge_pct
    async getGameSettings(gameId) {
      ...
      // Fallback hodnoty kdyz DB nefunguje
      const defaults = {
      ...
      try {
        ...
        const { data, error } = await sb
          .from('game_settings')
          .select('*')
          .eq('id', gameId)  // OPRAVA: 'id' ne 'game_id'
          .single();
  ...
  // ── INIT LOG ──────────────────────────────────────
  console.log('[CK] supabase.js v2.1 loaded');
  if (_session) {
    console.log('[CK] Prihlaseny:', _session.user.email);
    console.log('[CK] DB ops blokovany dokud hra nevola CK.setGameMode("real")');
  } else {
    console.log('[CK] Neprihlaseny hrac — demo mode, hraje zdarma bez DB');
  }
